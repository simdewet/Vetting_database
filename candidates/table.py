...
class CandidatesTable(tables.Table):
	...
	image_series = tables.TemplateColumn('{% if record.image_series %} <a href="{{record.image_series.url}}" target="_blank"> <img src="{{record.image_series.url}}"" width="50px"> </a>{% endif %}')
	light_curves = tables.TemplateColumn('{% if record.lightcurve %} <a href="{{record.lightcurve.url}}" target="_blank"> <img src="{{record.lightcurve.url}}"" width="100px"> </a>{% endif %}')
